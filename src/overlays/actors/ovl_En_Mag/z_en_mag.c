...
extern u8 D_0601EF00[]; // Logo Flame Effect
extern u8 D_0601F300[]; // "The Legend of"
extern u8 D_0601F540[]; // "Ocarina of Time"
...
void EnMag_DrawInner(Actor* thisx, GlobalContext* globalCtx, Gfx** gfxp) {
    ...
    static u8* effectMaskTextures[] = {
        0x0601A700, 0x0601AF00, 0x0601B700, 0x0601BF00, 0x0601C700, 0x0601CF00, 0x0601D700, 0x0601DF00, 0x0601E700,
    };
    EnMag* this = THIS;
    Font* font = &this->font;
    s32 pad;
    ...
    if ((s16)this->effectPrimLodFrac != 0) {
        for (k = 0, i = 0, rectTop = 0; i < 3; i++, rectTop += 64) {
            for (j = 0, rectLeft = 64; j < 3; j++, k++, rectLeft += 64) {
                EnMag_DrawEffectTextures(&gfx, effectMaskTextures[k], D_0601EF00, 64, 64, 32, 32, rectLeft, rectTop, 64,
    ...
    if ((s16)this->mainAlpha != 0) {
        EnMag_DrawImageRGBA32(&gfx, 160, 100, D_06000000, 160, 160);
    }
    ...
    if ((s16)this->mainAlpha != 0) {
        EnMag_DrawTextureI8(&gfx, D_0601F300, 72, 8, 154, 73, 72, 8, 1024, 1024);
        EnMag_DrawTextureI8(&gfx, D_0601F540, 96, 8, 152, 127, 96, 8, 1024, 1024);

        gDPPipeSync(gfx++);
        gDPSetPrimColor(gfx++, 0, 0, 200, 200, 150, (s16)this->mainAlpha);
        gDPSetEnvColor(gfx++, 100, 100, 50, 255);

        EnMag_DrawTextureI8(&gfx, D_0601F300, 72, 8, 153, 72, 72, 8, 1024, 1024);
        EnMag_DrawTextureI8(&gfx, D_0601F540, 96, 8, 151, 126, 96, 8, 1024, 1024);
